    ...
    <main className="relative min-h-screen bg-[#050505] text-[#f4f4f5] font-mono overflow-hidden p-12 md:p-24">
      ...
      <header className="relative z-10 mb-20">
        ...
        <h1 className="text-6xl md:text-8xl font-black tracking-tighter text-white uppercase leading-none mb-8">
          COMMAND<br/><span className="text-zinc-800">_ARCHIVE.</span>
        </h1>

        <div className="relative max-w-md">
          <input 
            type="text" 
            placeholder="GREP_ARCHIVE..."
            className="w-full bg-transparent border-b-2 border-zinc-800 px-0 py-3 text-xs font-bold outline-none focus:border-cyan-500 transition-all uppercase placeholder:text-zinc-800"
            value={search}
        ...
        {filtered.map((s, i) => (
          ...
          >
            ...
            <div className="relative mb-4 group/cmd">
              <code className="block bg-black p-4 border border-zinc-900 text-sm text-zinc-300 font-bold overflow-x-auto whitespace-pre group-hover/cmd:border-zinc-700 transition-colors">
                <span className="text-cyan-500 mr-2">$</span>{s.cmd}
              </code>
              <button 
                onClick={() => copyToClipboard(s.cmd)}
                className="absolute right-2 top-2 p-2 bg-zinc-900 text-zinc-500 opacity-0 group-hover/cmd:opacity-100 hover:text-white transition-all border border-zinc-800"
              >
                <Copy size={14} />
              </button>
            </div>
      ...
      {/* FOOTER */}
      <footer className="relative z-10 mt-40 pt-10 border-t border-zinc-900 flex justify-between items-center opacity-30">
        <div className="flex items-center gap-4 text-cyan-500">
          <Terminal size={24} />
          <span className="text-[10px] font-black uppercase tracking-widest">Snippet_Vault // Total: {filtered.length}</span>
        </div>
